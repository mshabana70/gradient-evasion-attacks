...
- **Limited Knowledge (LK)**: The goal of the adversary is the same, minimize $g(x)$ under the same constraints that each transformed attack point must remain within a maximum distance of $d_{max}$ from the corresponding original attack sample. However the attacker will not know the learned classifier $f$ or its training data $\mathcal{D}$, consequently not be able to compute $g(x)$. The adversary will instead have a surrogate dataset $`\mathcal{D'} = \{\hat{x}_{i}, \hat{y}_{i}\}_{i=1}^{n_{q}}`$ of $n_{q}$ samples from the same underlying distribution $p(\bm{X}, Y)$ that $\mathcal{D}$ was drawn from. The adversary approximates $`g(\bm{x})`$ as $`\hat{g}(\bm{x})`$, where $`\hat{g}(\bm{x})`$ is the discriminant function of the surrogate classifier $\hat{f}$ learnt on $`\bm{\mathcal{D'}}`$ An adversary will want $\hat{f}$ to closely approximate the targeted classifier $f$. So it stands to reason that instead of using the true class labels $\hat{y_{i}}$ to train $\hat{f}$, the adversary can query $f$ with sample from $\mathcal{D'}$ and train $\hat{f}$ using the labels $`\hat{y}_{i}^{c} = f(\hat{\bm{x}}_{i})`$ for each $`\bm(x)_{i}`$, resulting in an $\hat{f}$ that closely mimics $f$ on the surrogate data.
