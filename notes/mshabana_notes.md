...
1. **$\lambda = 0$**: Here the mission of the experiment is to transform the image of a handwritten "3" (a malicious sample) so that the classifier misclassifies it as a "7". With $\lambda = 0$, the attack solely focuses on minimizing $g(\textbf{x})$ (no need for $\hat{g}(\textbf{x})$ since this is a perfect knowledge scenario). The adversary's goal is purely push the sample across the decision boundary, making it misclassified, without any regard for how "natural" or "legitimate-looking" the modified sample becomes. This gives a visual effect that looks distorted and not something that resembles a "7" or a "3", which can be seen in the top row of Figure 3. So it's not the most stealthy approach if someone was visually looking at the samples. This type of attack can lead the gradient descent path into "unsupported areas" of the feature space (where $p(\textbf{x}) \approx 0$). In these regions, the classifier's behavior $g(\textbf{x})$ might be unconstrained by the training data, allowing for easy evasion without making the sample look like a "7". The classifier is evaded by making the attack sample sufficiently *dissimilar* from the malicious class, rather than similar to the target class.
2. **$\lambda > 0$**: Here, the additional mimicry term is introduced into the optimization objective, which estimates $p(\textbf{x}|y^c = -1)$ using a Kernel Density Estimator (KDE). As explained earlier, the adversary is encouraged to make the malicious sample *mimic* the characteristics of the legitimate sample. The visual outcome of this is shown in the bottom row of figure 3. The attack images, while still a modified "3", now strongly resemble a "7". The final attack image in that row closely looks like a mixture between a "3" and a "7". We also talked about the efficiency of this method, its slower than if $\lambda = 0$, or traditional gradient descent. 

The tradeoff here is apparent. Without mimicry, evasion can be quick but sloppy and easily identifiable. But with Mimicry, the attacks are guided towards regions that are more densely populated by legitimate samples, increasing the *confidence* that the evasion will be successful and that $g(\textbf{x})$ will be negative. However, this comes at a cost of requiring potentially more modifications or a "sub-optimal" gradient descent path, as the attacker is optimizing a more complex objective function. 

The author's also apply this gradient based attack when trying detect malware in PDF files. This is really to demonstrate the real-world implications of this attack, as opposed to the previous experiment being run on a toy dataset. The feature representation of the PDFs is using a technique from the following [paper](https://d1wqtxts1xzle7.cloudfront.net/111481365/maiorca_MLDM12-libre.pdf?1707982923=&response-content-disposition=inline%3B+filename%3DA_Pattern_Recognition_System_for_Malicio.pdf&Expires=1758241673&Signature=K1E78Rl~tjSos~IWfNBJpxBzcCU5Jt~P42ogXWLnvuvQzIaMX9Gg8XBNSlH2ht9eBEpDXkRN-RysIVEJPDehEpm0qb3JwzIsB9JslSbLH15A7ccyC2sKscuX6Fw~-xrO1EVHD9i8WRLUj8QedpXcAA2fmkg-s41BpctzUasqVfA9XQkX6J70hY9tUwLmz8qTmikvY8sgmktFu01nvQ67CUkQyVxltTMSeLe4nOw5lPYAz9nb5MjjZ19RjOAfdGrAMaK0Qt3IFB5yeWtwzoZGR5l9UE3EvbuxFb0mOiLeOyswwfX6gCVv8eGTb5Xq99D0GWsoDeCvE3sdmCVhiCnhYQ__&Key-Pair-Id=APKAJLOHF5GGSLRBV4ZA), wont go into the details of that though. At a high level, each feature of the PDF corresponds to the tally of occurences of a given keyword within the PDF file. Think of this as a bag-of-words model, but for PDF structural elements instead of textual words. For example, a keyword like `/JavaScript` or `/OpenAction` would contribute to a feature count. 

The maximum value of each feature (keyword count) was bounded to 100, as this value was found to be near the 95th percentile for each feature, limiting the influence of outliers. This is a step to regularize the feature space and prevent extreme values from dominating. 
